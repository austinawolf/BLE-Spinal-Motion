...
#define TARGET_UUID                 BLE_UUID_MOTION_SERVICE         	/**< Target device uuid that application is looking for. */

#define RTT_BUFFER_READ_LEN 128

#define UART_PREAMBLE 0xee
#define CHAR_LEN 20

// Create a FIFO structure
static app_fifo_t uart_fifo;
...
BLE_MOTION_C_DEF(m_motion_c);                                             /**< Structure used to identify the heart rate client module. */
...
static uint8_t serial_command_buffer[CHAR_LEN];
	
static void send_serial_command(void);
...
{
    ble_motion_on_db_disc_evt(&m_motion_c, p_evt);
 ...
 */
static void motion_c_evt_handler(ble_motion_c_t * p_motion_c, ble_motion_c_evt_t * p_motion_c_evt, void * p_context)
{
    ret_code_t err_code;

    switch (p_motion_c_evt->evt_type)
    {
        case BLE_MOTION_C_EVT_DISCOVERY_COMPLETE:
        {
            NRF_LOG_DEBUG("Quaternion Orientation service discovered.");

            err_code = ble_motion_c_handles_assign(p_motion_c,
                                                p_motion_c_evt->conn_handle,
                                                &p_motion_c_evt->params.peer_db);
            APP_ERROR_CHECK(err_code);

            // Initiate bonding.
            err_code = pm_conn_secure(p_motion_c_evt->conn_handle, false);
            if (err_code != NRF_ERROR_INVALID_STATE)
            {
                APP_ERROR_CHECK(err_code);
            }

            /* Motion service discovered. Enable notification of Heart Rate Measurement. */
            err_code = ble_motion_c_motionm_notif_enable(p_motion_c);
            APP_ERROR_CHECK(err_code);
        } break;
 ...
 */
static void motion_c_init(void)
{
...
int main(void)
{
    ...
    /* init uart fifo */
    err_code = app_fifo_init(&uart_fifo, fifo_buffer, fifo_buffer_size);    
    APP_ERROR_CHECK(err_code);
    ...
    err_code = app_fifo_init(&uart_fifo, fifo_buffer, fifo_buffer_size);    
    APP_ERROR_CHECK(err_code);
	NRF_LOG_INFO("Fifo Id: %p",&uart_fifo);		
    ...
    while (true)
    {
        ...
        if (app_fifo_get(&uart_fifo, &byte) == NRF_SUCCESS) {
            app_uart_put(byte);
        }	
    }
}
...
static void send_serial_command(void) {
    ...
    NRF_LOG_DEBUG("Send Serial Command Call.");

    memset(serial_command_buffer, 0, CHAR_LEN);

    serial_command_buffer[i++] = COMMAND_PREAMBLE;
    ...
    while (true) {
        ...
        {				
            ...
            if (i > CHAR_LEN) {
                NRF_LOG_ERROR("Characteristic Length Exceeded");
                return;
            }
                    
        }
        if (timeout_clock++ > 50) {
            NRF_LOG_ERROR("Serial Command Read Timeout");
            return;
        }	
        nrf_delay_ms(1);
