/**@file
 *
 * @defgroup ble_motion_c Heart Rate Service Client
 * @{
 * @ingroup  ble_sdk_srv
 ...
 *          @code
 *              ble_motion_c_t instance;
 *              NRF_SDH_BLE_OBSERVER(anything, BLE_MOTION_C_BLE_OBSERVER_PRIO,
 *                                   ble_motion_c_on_ble_evt, &instance);
 *          @endcode
 */

#ifndef BLE_MOTION_C_H__
#define BLE_MOTION_C_H__
...
/**@brief   Macro for defining a ble_motion_c instance.
 *
 * @param   _name   Name of the instance.
 * @hideinitializer
 */
#define BLE_MOTION_C_DEF(_name)                                                                        \
static ble_motion_c_t _name;                                                                           \
NRF_SDH_BLE_OBSERVER(_name ## _obs,                                                                 \
                     BLE_MOTION_C_BLE_OBSERVER_PRIO,                                                   \
                     ble_motion_c_on_ble_evt, &_name)

/** @brief  Maximum number of RR intervals to be decoded for each MOTIONM notifications (any extra RR intervals will be ignored).
 *
 * This define should be defined in the sdk_config.h file to override the default.
 */
#ifndef BLE_MOTION_C_RR_INTERVALS_MAX_CNT
#define BLE_MOTION_C_RR_INTERVALS_MAX_CNT 20
#endif


/**
 * @defgroup motion_c_enums Enumerations
 * @{
 */

/**@brief MOTION Client event type. */
typedef enum
{
...
/**
 * @defgroup motion_c_structs Structures
 * @{
 */
...
typedef struct
{
    ble_motion_c_evt_type_t evt_type;    /**< Type of the event. */
    uint16_t             conn_handle; /**< Connection handle on which the Heart Rate service was discovered on the peer device..*/
    union
    {
        motion_db_t  peer_db;            /**< Heart Rate related handles found on the peer device.. This will be filled if the evt_type is @ref BLE_MOTION_C_EVT_DISCOVERY_COMPLETE.*/
        ble_motionm_t motionm;                /**< Heart rate measurement received. This will be filled if the evt_type is @ref BLE_MOTION_C_EVT_MOTIONM_NOTIFICATION. */
    } params;
} ble_motion_c_evt_t;
...
/**
 * @defgroup motion_c_types Types
 * @{
 */

// Forward declaration of the ble_bas_t type.
typedef struct ble_motion_c_s ble_motion_c_t;
 ...
 */
typedef void (* ble_motion_c_evt_handler_t) (ble_motion_c_t * p_ble_motion_c, ble_motion_c_evt_t * p_evt, void * p_context);

/** @} */

/**
 * @addtogroup motion_c_structs
 * @{
 */
...
typedef struct
{
    ble_motion_c_evt_handler_t evt_handler;  /**< Event handler to be called by the Heart Rate Client module whenever there is an event related to the Heart Rate Service. */
    app_fifo_t * p_uart_fifo;
} ble_motion_c_init_t;
...
/**
 * @defgroup motion_c_functions Functions
 * @{
 */
 ...
 *
 * @param[in] p_ble_motion_c      Pointer to the heart rate client structure.
 * @param[in] p_ble_motion_c_init Pointer to the heart rate initialization structure containing the
 ...
 *
 * @param   p_ble_motion_c Pointer to the heart rate client structure.
 *
 ...
 *
 * @param[in] p_ble_motion_c Pointer to the heart rate client structure instance to associate.
...
/**@brief     Function for assigning a handles to a this instance of motion_c.
 *
 ...
 *            provided from the discovery event @ref BLE_MOTION_C_EVT_DISCOVERY_COMPLETE.
 *
 * @param[in] p_ble_motion_c        Pointer to the heart rate client structure instance to associate.
 * @param[in] conn_handle        Connection handle to associated with the given Heart Rate Client Instance.
 * @param[in] p_peer_motion_handles Attribute handles for the MOTION server you want this MOTION_C client to
 *                               interact with.
 */
